...
1. If `AGY_CODEX_PLUGIN_ROOT` is set, use `$AGY_CODEX_PLUGIN_ROOT/scripts/agy-codex.mjs`.
2. If the current workspace contains `scripts/agy-codex.mjs`, use that.
3. Otherwise use the installed plugin path:
   - Windows: `%USERPROFILE%\.gemini\antigravity-cli\plugins\antigravity-codex\scripts\agy-codex.mjs`
   - macOS/Linux: `$HOME/.gemini/antigravity-cli/plugins/antigravity-codex/scripts/agy-codex.mjs`
