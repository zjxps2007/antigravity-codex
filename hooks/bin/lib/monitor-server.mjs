...
function monitorCommandLine(scriptPath, host, port) {
    return [
        process.execPath,
        scriptPath,
        "monitor-server",
...
function startWindowsMonitorWithCim(scriptPath, host, port) {
...
function startWindowsMonitorWithStartProcess(scriptPath, host, port) {
    const command = [
        "$ErrorActionPreference = 'Stop'",
...
function startWindowsMonitorProcess(scriptPath, host, port) {
    try {
        return startWindowsMonitorWithCim(scriptPath, host, port);
    }
    catch (error) {
        process.stderr.write(`Win32_Process.Create monitor launch failed; falling back to Start-Process: ${error instanceof Error ? error.message : String(error)}\n`);
        return startWindowsMonitorWithStartProcess(scriptPath, host, port);
    }
}
function startDetachedMonitorProcess(scriptPath, host, port) {
    if (process.platform === "win32") {
