...
function inspectAntigravity() {
    ...
    const codexImport = imports.find((item) => {
        return typeof item === "object" && item !== null && item.name === "codex";
    });
    const hooks = readJsonObject(installedHooksFile);
    const stopHook = hooks?.["codex-stop-review-gate"];
    const stopHookObject = typeof stopHook === "object" && stopHook ? stopHook : null;
    const stopEntries = Array.isArray(stopHookObject?.Stop) ? stopHookObject.Stop : [];
    const commandEntry = stopEntries.find((item) => {
        return typeof item === "object" && item !== null && item.type === "command";
    });
...
function hookScriptPath(rootDir) {
...
function withTemporaryDataRoot(dataRoot, fn) {
...
function runHookSmokeTest(cwd, rootDir, activeHookCommand, installedHookCommand) {
    const script = hookScriptPath(rootDir);
    const commandToRun = activeHookCommand || (script ? `${process.execPath} ${script}` : null) || installedHookCommand;
    if (!commandToRun) {
        return {
            status: "fail",
            command: "",
...
function buildChecks(node, codex, git, antigravity, activeHook, reviewGate, hookSmokeTest) {
    ...
    addCheck(checks, "Codex CLI", codex ? (codex.available ? "pass" : "fail") : "skip", codex ? (codex.available ? codex.stdout : codex.error ?? codex.stderr ?? "missing") : "not checked");
    addCheck(checks, "Git workspace", git.isRepo ? "pass" : "fail", git.root ?? git.error ?? "not a git repository");
    addCheck(checks, "Git changes", git.isRepo && git.hasChanges ? "pass" : "warn", git.isRepo
        ? git.hasChanges
    ...
    addCheck(checks, "Review gate config", reviewGate.enabled ? "pass" : "fail", reviewGate.enabled ? `enabled at ${reviewGate.configDir}` : `disabled at ${reviewGate.configDir}`);
...
export function buildDoctorReport(cwd, options) {
