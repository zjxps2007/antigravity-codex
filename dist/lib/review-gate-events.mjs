...
function eventMatchesWorkspace(event, workspaceRoot) {
    if (!workspaceRoot)
        return true;
    if (!event.workspace)
        return false;
...
export function readReviewGateEvents(limit = 200, workspaceRoot) {
...
export function clearReviewGateEvents(workspaceRoot) {
    try {
        if (workspaceRoot) {
            const file = reviewGateEventsFile();
            if (!fs.existsSync(file))
