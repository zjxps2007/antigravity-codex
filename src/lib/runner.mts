...
export function createLogFile(cwd: string, jobId: string): string {
  const dir = jobDir(cwd, jobId);
  fs.mkdirSync(dir, { recursive: true });
  return path.join(dir, "log.txt");
}

export function killProcessTree(pid: number | null | undefined): void {
  if (!pid) return;
...
export function printQueued(job: Job, asJson = false): void {
  const payload = {
    jobId: job.id,
    status: "queued",
    title: job.title,
...
export async function executeTrackedRequest(
  request: CodexRequest,
  job: JobIdentity,
  ...
  const status: JobStatus = isCancelled
    ? "cancelled"
    : result.code === 0
      ? "completed"
      : "failed";
...
export function queueBackground(request: CodexRequest, scriptPath: string): Job {
  ...
  const child = spawn(
    process.execPath,
    [scriptPath, "worker", "--cwd", request.cwd, "--job-id", jobId],
    {
      cwd: request.cwd,
      detached: true,
      stdio: "ignore",
...
export async function runForeground(request: CodexRequest): Promise<void> {
...
export async function runMaybeBackground(
  request: CodexRequest,
